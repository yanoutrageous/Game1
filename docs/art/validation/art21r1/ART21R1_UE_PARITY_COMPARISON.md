...
| run_hud | Room/player/object world layer is primary; left rail fixed; right top card compact; bottom key bar overlays without cutting the room away. | Restores room-specific background through `PresentationMapping`, removes permanent room text masks, hides the right fill layer, and shows feedback only on command state. | PASS | Left minimap is larger and more table-like than UE. Bottom key bar remains heavy. |
...
ART-21R1 improves the most severe visual failure: Run HUD no longer treats the room as a background behind a generated UI frame. Main menu is cleaner while preserving its original deploy click route. Map and modals are usable.
