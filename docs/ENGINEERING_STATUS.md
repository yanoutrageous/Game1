...
G25 adds a clear main-menu `快速开始 / Demo Run` current playable route entry, keeps DeployPrep start preview-only, improves DeployPrep / LongTerm readability, and reduces Settings placeholder noise. It does not implement real warehouse, rewards, settlement, gacha, objectives, red dots, SaveManager, asset writes, real LongTerm backend, real settings, or art import. Gameplay runtime and manual playtest were not run and are not claimed as PASS.
...
G24 remains foundation-only. It is not complete LongTerm, real targets, real rewards, real gacha, real red dots, real SaveManager, real asset writes, complete Warehouse, or complete Gacha.
...
G24 adds LongTerm Content Framework Foundation only: six-module content framework, secondary groups, preview cards, Objective / Reward / Gacha / Collection preview slots, and UI / art / data key reservation. It does not implement real task systems, achievement systems, commission settlement, reward claiming, claim, red dot clearing, gacha probability / pity / cost / result grant, cosmetic configuration, unique collectible acquisition, codex unlock, research unlock, profile progression, history writes, SaveManager, event bus, asset writes, complete LongTerm, complete Warehouse, or complete Gacha. Gameplay runtime and manual playtest were not run and are not claimed as PASS.
...
- G20-R3d2 status: decision log, glossary, temporary / deprecated inventory, SOURCE_REGISTRY minimal status update, and navigation references added in this batch
...
- `docs/validation/G21_ASSET_ITEM_FLOW_CONTRACT_VALIDATION.md` and `docs/handoff/HANDOFF_G21_ASSET_ITEM_FLOW_CONTRACT.md` record G21 contract-only scope and validation boundary.
