...
Status: implementation in progress on `godot/m2-latest-planning-minimum-gameplay-meta-loop`.

Current M2 scope: align existing M1 playable loop to latest planning minimum gameplay and meta loop. DeployPrep starts the existing `standard_10x10` route through RunStartConfig / route adapter; RunScene keeps TruthMap / IntelMap separation; RunResult is settlement input; LongTerm consumes MetaProgress and latest result display-only.
