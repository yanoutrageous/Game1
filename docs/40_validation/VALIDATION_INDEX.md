...
| I4.1–I4.3 隔离/诊断/数量 | TARGETED PASS / VISUAL CANDIDATE | sandbox/taint、确定性场景夹具、一次性保存失败注入、诊断身份/读写分区、默认档哈希、原子 N 件购买和精确实例数量定向门通过；12 组测试场捕获完成，外部动态门未过。 |
| I4.4 Deploy | TARGETED PASS / VISUAL CANDIDATE | 原反例继续保留；当前候选已通过两行卡、单一数量语义、310/12/310、六项摘要、滚动和上下文金币门，12 张真实 Deploy 原图完成逐图复核。 |
| I4.5 局内补充 | TARGETED PASS / VISUAL CANDIDATE | 地图四层、稳定阻挡 descriptor/可见足迹、纹理 fallback、内容驱动左下高度、品质描述器与世界掉落光束定向门通过；12 组机器捕获完成，逐原图/动态玩家门未过。 |
| I4.6 长期导航 | TARGETED PASS / VISUAL CANDIDATE | 通知直达、显式已读、页面历史及筛选/选择/滚动/焦点恢复 runner 通过；6 模块/25 页面真实矩阵已逐图复核。 |
| I4.7 字体/边框/HUD | TARGETED PASS / VISUAL CANDIDATE | display/readable 均为 FusionPixel 主字体、Noto 仅缺字回退；16/8/4/2 边框、协议安全区和 0/1/3/4/满包布局定向门通过；正确字体 12 组机器矩阵完成，逐图/动态门未关闭。 |
| I4 post-audit worktree full | PASS / EXTERNAL ACCEPTANCE BLOCKED | I1 full 104/104：报告 SHA-256 `DCC08579…62B2D`；I4 full：`A7C47915…9DDB8`；静态 12/12、I4 runner 9、protected dirty 0、fixed-frame 0。自动总门保持 `BLOCKED_EXTERNAL_DEVICE_AND_DYNAMIC_ACCEPTANCE`。 |
| I4 旧断言全量处置 | PASS | 入口以来既有测试文件 15/15、38 条 disposition、19 条带替代门的 superseded、0 invalid；最新报告 SHA-256 `CA7047D5…6E35`。 |
| I4 正确字体全内容矩阵 | CAPTURE COMPLETE / VISUAL CANDIDATE | 156 行 × 12 组 = 1872 个覆盖单元，1140 张原尺寸 PNG；manifest SHA-256 `00FFB0BF…6C293`，worktree unchanged。逐原图人工与动态玩家验收未执行。 |
| I4 错误字体中止矩阵 | INVALID / FORBIDDEN AS PROOF | `.tmp/i4/census_matrix/20260730T055045676Z` 使用了违反阶段权威的 Noto 正文主字体；已停止精确验证进程树并登记 `INVALID_WRONG_FONT_AUTHORITY`，不得计入 PASS。 |
...
| I4 实现候选 exact-head/full | PASS / EXTERNAL ACCEPTANCE BLOCKED | 候选 `f950eefb000ab298344059dfa8afc125aa79ed8a`；总报告 SHA-256 `DAFE502175D30034C210AA664769770AB74420AF42FB1B45BE98F4E15F079E56`；静态 `C708B0BC…`、census `CC31EB55…`、重复 6×10+3 `5B7805C7…`、设备 `48335E1D…`。 |
| I4 实现候选 exact-head 真实捕获 | VISUAL CANDIDATE | 同一 `f950eef` 下 51 图，manifest SHA-256 `981126BEFFC89311986262EB78A036DE054911DE2869982BD0F99B49F5F8800E`；0 重复语义 SHA、工作树未变。阶段审计回填形成新 HEAD 后须重跑 exact-head/full。 |
| I4 最终候选首次 I1 exact-head | FAIL PRESERVED / REMEDIATED IN NEXT CANDIDATE | `fafbbff` 为 103/104；满包替换业务 marker/退出码均 PASS，但退出清理偶发 19 而合同为 18，报告 SHA-256 `5DAA2B78…73FC`。未放宽合同；主场景释放后的退栈等待 2→4，修改后 3 个新进程均业务 PASS 且精确 18，最终候选仍须全量 head 复验。 |
...
| I4 C/E 存储收口 | PHASE1 PASS / FINAL REMEASURE PENDING | 精确目标清理实际释放 E 盘 19,342,217,216 字节；C 盘审计后零删除。正确字体矩阵 156×12/1140 PNG、最终/失败报告、来源、用户档、stash、CAS/index/恢复证明均保留；最终 exact-head 后末轮复量。 |
