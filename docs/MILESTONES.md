...
- G20 docs-only governance completed R4A read-only acceptance, R4B docs-only closeout, fast-forward main merge, and post-merge docs calibration. G21-R3 has started separately on `godot/g21-asset-item-flow-contract` as Asset & Item Flow Contract Foundation and still requires independent validation before any main merge decision.
