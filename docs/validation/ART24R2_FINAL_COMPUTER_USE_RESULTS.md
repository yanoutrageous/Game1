...
- 验收日期：2026-07-19；当前快照复验：2026-07-20
- 事实来源：UE 源码与实机、Godot 源码与合并 M7 后的生产 `main.tscn` 实机
...
冻结标准要求 61 个二级状态全部经 Computer Use 通过后才允许标记验收完成。本轮未满足该标准。用户于 2026-07-19 首次要求停止扩大返工，并于 2026-07-20 在合并 M7、恢复检查后再次明确要求把当前内容跑完，以“验收部分不通过”的状态结束并 push。因此：
...
本次复验在任何验收操作前继续沿用已经冻结的 `ART24R2-UE-LAYOUT-CU-FROZEN-1` 与 `ART24R2-FINAL-CU-FROZEN-1`，没有重新定义或放宽标准。Computer Use 从生产 `main.tscn` 完成了以下真实路线：

1. 主菜单进入“出发探索”；
2. 在合并 M7 后的真实出发页确认 7×7 简单地图并进入局内；
3. 观察方形房间、当前角色尺寸、左栏、协议层和底栏；
4. 输入 `M` 打开真实 7×7 地图，点击未知格放置标记；
5. 再次输入 `M` 关闭地图并返回同一局内状态。

该路线证明 M7 合并没有破坏已有的主流程与 `map.overview` / `map.marked` / 地图关闭返回表现，但没有生成 `map.return_available`、失败保全、多物品、怪物五态及完整/减弱动效的新鲜实机证据。因此统计保持 `PASS 24 / FAIL 37`，不以代码实现或自动化探针代替剩余视觉验收。
...
| 地图 | 全屏暗幕、大幅主网格、底部反馈，未知格可标记 | 移除常驻详情条，集中布局，标记语义独立；M7 的 7×7 / 10×10 / 13×13 使用同一自适应布局 | 7×7 概览、选格、标记及关闭返回在当前快照复验通过；已探索格快速返回仍未形成冻结证据 | FAIL |
...
- 2026-07-20 当前快照关闭前复跑：`ART24_IN_RUN_RUNTIME=PASS primary_modules=8 secondary_states=61 required_visual_keys=33`、`ART24_MAP_OVERLAY_LAYOUT=PASS resolutions=5 cells=100 safe_bottom=reserved`、`M7_META_UI_RUNTIME:PASS`、`G41_IN_RUN_CORE_GAMEPLAY_RUNTIME=PASS`。
- M7 八地图 × 100 种子的压力 runner 已在同一合并头先前得到 `PASS`；关闭阶段时的额外重复运行超过命令捕获窗口，未取得新的可引用结论，因此没有用它覆盖既有 PASS，也没有将其解释成新的失败。
