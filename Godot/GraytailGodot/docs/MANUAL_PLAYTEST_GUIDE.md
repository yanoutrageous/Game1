...
G20 docs-only note: R3a imported authorized text design source copies, R3b adds project governance/index documents, R3c adds G10-G19 stage summaries plus route analysis, R3d1 adds branch / commit / validation governance matrices, and R3d2 adds decision log / glossary / temporary-deprecated inventory. Do not record these docs-only batches as `Godot headless project-load/parser smoke PASS`, `gameplay runtime PASS`, or `manual playtest PASS`. G20 is not merged to main, G20 closeout has not executed, G21 has not started, and any G21+ work requires separate authorization and its own validation record.
