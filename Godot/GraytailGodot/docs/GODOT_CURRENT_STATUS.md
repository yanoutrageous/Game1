...
G20 docs-only governance branch: `godot/g20-project-knowledge-governance`. R3a imported authorized text design source copies under `docs/design_sources/`; R3b adds project governance maps and design source indexes; R3c adds G10-G19 stage summaries, route analysis, system boundary map, and stage dependency map; R3d1 adds branch, commit, and validation governance matrices under `docs/project_governance/`. G20 is not merged to main, G20-R3d2 has not executed, G21 has not started, and no Godot project, scene, resource, font, import product, `.uid`, or `.translation` change is part of R3a/R3b/R3c/R3d1.

G20-R3a/R3b/R3c/R3d1 Godot run status: not run. No Godot headless project-load/parser smoke PASS, gameplay runtime PASS, or manual playtest PASS is claimed for these docs-only batches.
