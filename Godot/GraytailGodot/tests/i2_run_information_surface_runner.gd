...
func _run() -> void:
	...
	await _check_runtime_mine_risk_pipeline(surface, profile)
	canvas.queue_free()
	await _frames(2)
	_finish()
...
func _check_read_only_descriptors() -> void:
	...
	var direct_feedback := RunSurfaceModelScript.build({
		"current_room": &"Normal",
		"last_message": "进入普通房间。",
	}, null, {}, {})
	_check(String(direct_feedback.get("command_feedback", "")).is_empty(), "Persistent run model restored a stale command-feedback strip")
	var player_copy_cases := [
		["Monster present. Fight is available.", "发现异常体"],
		["Chest can be searched.", "发现物资箱"],
		["Exit room ready. Request extraction.", "已抵达撤离点"],
		["This room was already searched.", "已搜索"],
		["Triggered mine re-entered; no damage.", "不会再次受伤"],
		["Extraction requested. Confirm or cancel.", "准备撤离"],
		["Combat hit: -2 HP.", "受到攻击"],
		["Fled combat: lost 3 pending black coin; 1 item(s) left on this room floor.", "已撤出战斗"],
		["Altar exchange complete: HP -10, black_coin +8, item +1.", "祭坛交换完成"],
		["Trader info: black_coin -2, nearby clue recorded.", "情报交易完成"],
		["Mechanism triggered: HP -1, pressure +5.", "机关触发"],
	]
	for player_copy_case in player_copy_cases:
		var raw_message := String(player_copy_case[0])
		var expected_copy := String(player_copy_case[1])
		var player_copy := RunUIViewModelScript.player_message(raw_message)
		_check(player_copy.contains(expected_copy), "Player-facing message mapping leaked or lost copy for: %s" % raw_message)
		var projected := RunSurfaceModelScript.build({
			"current_room": &"Normal",
			"last_message": raw_message,
		}, null, {}, {})
		_check(String(projected.get("command_feedback", "")).is_empty(), "Persistent run model restored a transient command result for: %s" % raw_message)
...
func _check_quick_bag(surface: RunSurface, profile: Dictionary) -> void:
	for item_count in [0, 1, 4, 5, 8]:
		...
		for index in range(buttons.size()):
			...
			var marker := button.find_child("BackpackItemRarityMarker", false, false) as ColorRect
			_check(marker != null and marker.color.is_equal_approx(Color(rarity.get("color"))), "Quick bag row %d rarity marker drifted" % index)
			_check(marker != null and marker.mouse_filter == Control.MOUSE_FILTER_IGNORE, "Quick bag row %d rarity marker intercepted input" % index)
			var normal_style := button.get_theme_stylebox("normal") as StyleBoxFlat
			_check(normal_style != null and not normal_style.border_color.is_equal_approx(Color(rarity.get("color"))), "Quick bag row %d still uses a full rarity border" % index)
			var level := int(item.get("collectible_level", 0))
			_check(button.text.contains("收藏等级 %d" % level) if level > 0 else not button.text.contains("收藏等级"), "Quick bag row %d collectible-level visibility drifted" % index)
		if item_count > 0:
...
func _check_mine_risk_and_protocol(surface: RunSurface, profile: Dictionary) -> void:
	for adjacent in [-1, 0, 1, 3, 6, 8]:
		...
		_check(not surface.resource_label.visible, "Mine-risk copy remained duplicated in the left rail for %d" % adjacent)
		_check(surface.mine_risk_tag_art.visible == (adjacent >= 0), "Bottom mine-risk plate visibility drifted for %d" % adjacent)
		_check(surface.mine_risk_label.visible == (adjacent >= 0), "Bottom mine-risk text visibility drifted for %d" % adjacent)
		if adjacent >= 0:
			_check(surface.mine_risk_label.text == "周围雷险：%d" % adjacent, "Bottom mine-risk count drifted for %d" % adjacent)
	...
	for level in range(1, 6):
		...
		await _frames(1)
		_check(surface.right_body_label.text.contains(expected_titles[level]), "HUD omitted protocol title at level %d" % level)
...
func _check_runtime_mine_risk_pipeline(surface: RunSurface, profile: Dictionary) -> void:
	var cases := [
		{
			"id": "diagonal",
			"spawn": Vector2i(1, 1),
		...
		},
		{
			"id": "boundary",
			"spawn": Vector2i(0, 0),
			"mines": [Vector2i(1, 0), Vector2i(0, 1), Vector2i(1, 1)],
			"expected": 3,
		},
		{
			"id": "zero",
	...
	for test_case in cases:
		...
		context.start_run({
			"id": "i2_mine_risk_%s" % String(test_case.get("id", "")),
			"width": 3,
			"height": 3,
			"manual_map": {
				"spawn": test_case.get("spawn", Vector2i.ZERO),
				"mines": test_case.get("mines", []),
				"events": [],
				"monsters": [],
				"chests": [],
				"exits": [],
		...
		var expected := int(test_case.get("expected", -1))
		var computed: int = int(context.minefield_service.count_adjacent_mines(context.truth_map, context.player_pos))
		_check(computed == expected, "TruthMap/MinefieldService count drifted for %s" % String(test_case.get("id", "")))
		var snapshot: Dictionary = context.get_status_snapshot()
		_check(int(snapshot.get("adjacent_mines", -1)) == expected, "Runtime snapshot lost mine-risk count for %s" % String(test_case.get("id", "")))
		var model := RunSurfaceModelScript.build(snapshot, null, profile, {})
		surface.apply_surface_model(model)
		await _frames(1)
		_check(surface.mine_risk_label.visible, "Bottom mine-risk label hid a known %s count" % String(test_case.get("id", "")))
		_check(surface.mine_risk_label.text.contains(str(expected)), "Bottom mine-risk label lost %s count" % String(test_case.get("id", "")))
		last_snapshot = snapshot
	var unknown_snapshot := last_snapshot.duplicate(true)
	unknown_snapshot.erase("adjacent_mines")
	var unknown_model := RunSurfaceModelScript.build(unknown_snapshot, null, profile, {})
	surface.apply_surface_model(unknown_model)
	await _frames(1)
	_check(not bool((unknown_model.get("mine_risk", {}) as Dictionary).get("known", true)), "Missing runtime mine-risk data masqueraded as zero")
	_check(not surface.mine_risk_label.visible, "Unknown runtime mine-risk rendered as a zero-risk label")
...
func _sample_items(count: int) -> Array[Dictionary]:
	...
	for index in range(count):
		...
		if index < 6:
			item["collectible_level"] = index + 1
		items.append(item)
	return items
...
func _finish() -> void:
	if failures.is_empty():
		print("I2_RUN_INFORMATION_SURFACE=PASS quick_bag=0,1,4,5,8 scroll=all_items hover_focus=display_only mine_risk=unknown,0-8 protocol=levels_1-5 rarity_marker=independent collectible_level=authoritative")
		quit(0)
		return
