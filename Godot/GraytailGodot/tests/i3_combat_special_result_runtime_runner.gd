...
func _check_combat_presentation_sequence() -> void:
	...
	var appearance_duration := float(actor.appearance_snapshot().get("duration", 0.0))
	actor._process(appearance_duration + 0.01)
	var completed_appearance: Dictionary = actor.appearance_snapshot()
	_require(is_equal_approx(float(completed_appearance.get("progress", 0.0)), 1.0), "enemy entry did not complete after its authored duration")
	_require(not bool(completed_appearance.get("cue_visible", true)), "enemy entry cue remained visible after its authored duration")
