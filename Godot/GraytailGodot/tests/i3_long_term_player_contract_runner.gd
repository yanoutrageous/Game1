...
func _test_player_copy(shell: Control) -> void:
	for module_id in [&"task_archive", &"codex", &"research", &"profile", &"collection_appearance"]:
...
func _test_task_archive_boundary(shell: Control) -> void:
	...
	_check(not (shell.get("tab_buttons") as Dictionary).has(&"talent"), "A new talent authority was fabricated")
...
func _finish() -> void:
	if failures.is_empty():
		print("I3_LONG_TERM_PLAYER_CONTRACT=PASS source=m7_research_prerequisite talent_rules=0 tree_nodes=3 player_copy=clean")
		quit(0)
		return
