...
func _draw() -> void:
	for obstacle in logical_obstacles:
		var world_rect := Rect2(
			...
			Vector2(obstacle.size.x * ROOM_RECT.size.x, obstacle.size.y * ROOM_RECT.size.y)
		)
	...
	if bool(combat_snapshot.get("active", false)):
		...
		draw_line(Vector2(420, 400), Vector2(438, 400), color, 4.0)
		draw_line(Vector2(842, 400), Vector2(860, 400), color, 4.0)
		draw_line(Vector2(640, 220), Vector2(640, 238), color, 4.0)
		draw_line(Vector2(640, 562), Vector2(640, 580), color, 4.0)
