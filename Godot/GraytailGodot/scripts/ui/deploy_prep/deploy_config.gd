...
static func right_summary_preview(config: Dictionary) -> Dictionary:
	...
	return {
		"summary": [
			...
			"map=%s difficulty=%s region=%s" % [str(config.get("map_mode_label", "")), str(config.get("difficulty_label", "")), str(config.get("region_label", ""))],
			"objective=%s" % str(objective.get("selected_label", config.get("selected_objective_label", ""))),
			"capacity=%s" % str(capacity.get("label", "")),
			"validity=%s" % str(validity.get("label", "")),
			"active_run=%s" % str(active_run.get("label", "")),
		],
		"config": [
			"selected_equipment=%s" % _join_array(loadout.get("equipped", []), "none"),
			"selected_consumables=%s" % _join_array(loadout.get("carried_consumables", []), "none"),
			"bag=%d/%d" % [int(config.get("bag_used", 0)), int(config.get("bag_limit", 10))],
			"warehouse_items=%d" % int(warehouse.get("item_count", 0)),
		],
		"effect": [
			"equipment enters equipped/runtime passive context on next run",
			"consumables enter the next run backpack and are consumed through existing command flow",
			"unused consumables return on success and become failure salvage candidates",
			"profile/talent hooks expose backpack, salvage, and mine-damage minimal effects",
		],
		"risk": [
			"no full deploy bootstrapper",
			"no complete warehouse economy",
			"no complete objective/reward/pool system",
			"continue/abandon active-run persistence remains future work",
		],
	}
