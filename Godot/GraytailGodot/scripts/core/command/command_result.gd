...
static func rejected(command: Dictionary, reason_code: String, message_key: String = "", produced_events: Array = [], produced_transactions: Array = [], snapshot_delta: Dictionary = {}) -> Dictionary:
	var key: String = message_key if message_key != "" else "command.rejected.%s" % reason_code
...
static func from_action(command: Dictionary, action_result: Dictionary, produced_events: Array = [], produced_transactions: Array = [], snapshot_delta: Dictionary = {}) -> Dictionary:
	var accepted_result: bool = bool(action_result.get("ok", false))
	var reason_code: String = "" if accepted_result else String(action_result.get("reason", action_result.get("blocked_reason", "blocked")))
	var default_message_key: String = "command.accepted" if accepted_result else "command.rejected.%s" % reason_code
	var default_status: StringName = &"accepted" if accepted_result else &"blocked"
	var message_key: String = String(action_result.get("message_key", default_message_key))
	var result: Dictionary = _base_result(command, accepted_result, reason_code, message_key, produced_events, produced_transactions, snapshot_delta)
...
static func _base_result(command: Dictionary, accepted_value: bool, reason_code: String, message_key: String, produced_events: Array, produced_transactions: Array, snapshot_delta: Dictionary) -> Dictionary:
	var status_value: StringName = &"accepted" if accepted_value else &"blocked"
	return {
