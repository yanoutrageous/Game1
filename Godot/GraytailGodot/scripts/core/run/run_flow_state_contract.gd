...
static func build_abandon_intent_preview(context: RunContext = null) -> Dictionary:
	...
	return {
		...
		"gold_coin_state": &"retained",
		"safe_yield_state": &"retained",
		"boundary": "Abandon is a strong-confirm intent that routes to the real settle_abandon branch: run black coin and all items are lost, salvage capacity is zero, and direct gold is retained.",
		"read_only": true,
		"display_only": true,
		"preview": true,
	}
