...
const ART21_COMPONENT_BY_VISUAL_KEY := {
	...
	&"run.status_card.frame": &"ui.art21.run.status_card.frame",
	&"run.bottom_overlay.frame": &"ui.art21.run.bottom_overlay.frame",
...
const MAP_VISUAL_KEY_BY_STATE := {
	...
	&"flagged": &"map_overlay.cell.flagged.replaced",
	&"event": &"map_overlay.marker.event.replaced",
