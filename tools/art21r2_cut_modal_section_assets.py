#!/usr/bin/env python3
"""Cut ART21R2 modal title/section assets from Zujian3 candidates."""
...
CUT_ROOT = ART_ROOT / "03_cut_output" / "modal_sections"
...
class SectionCut:
    ...
    @property
    def cut_id(self) -> str:
        return f"art21r2_modal_section_{self.role}"
...
SECTION_CUTS = [
    SectionCut(
        ...
        "ui_art21r2_modal_title_plate.png",
        34,
        10,
    ),
    SectionCut(
        ...
        "ui_art21r2_modal_section_panel.png",
        32,
        8,
    ),
    SectionCut(
        ...
        "Zujian3_candidate_012.png",
        "ui_art21r2_modal_action_strip.png",
        34,
        10,
    ),
]
...
def cut_section_image(path: Path) -> tuple[Image.Image, dict[str, object], dict[str, object]]:
...
def build_rows(write_runtime: bool, force: bool) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], dict[str, object]]:
    ...
    for item in SECTION_CUTS:
        ...
        cleaned, before_stats, after_stats = cut_section_image(candidate_path)
        ...
        staging_rows.append(
            {
                ...
                **before_stats,
                ...
                "reason": "Zujian3 modal section candidate requires purple-fringe cleanup before runtime import.",
            }
        )
        ...
        row = {
            ...
            "note": "Draw-sliced modal title/section surface; does not claim final modal composition.",
        }
...
def main() -> int:
    ...
    write_csv(MANIFEST_ROOT / "modal_section_staging_manifest.csv", staging_rows, STAGING_FIELDS)
    write_csv(MANIFEST_ROOT / "modal_section_cut_dry_run_plan.csv", plan_rows, PLAN_FIELDS)
    write_csv(MANIFEST_ROOT / "modal_section_cut_manifest.csv", cut_rows, CUT_FIELDS)
    summary_path = MANIFEST_ROOT / "modal_section_cut_summary.json"
